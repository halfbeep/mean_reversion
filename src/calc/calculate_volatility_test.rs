...
fn test_calculate_volatility_with_random_prices() {
    ...
    let results_map: Arc<RwLock<HashMap<NaiveDateTime, (Option<f64>, Option<f64>)>>> =
    ...
    for i in 0..50 {
        ...
        // Insert the random price into the HashMap within the RwLock
        {
            ...
            map.insert(timestamp, (Some(random_price), None));
        }
    }
...
fn test_interpolation_in_calculate_volatility() {
    ...
    {
        let mut map = results_map.write().unwrap();
        map.insert(base_timestamp, (Some(100.0), None));
        map.insert(base_timestamp + Duration::seconds(1), (Some(110.0), None));

        // Introduce gaps with None values that should be interpolated
        map.insert(base_timestamp + Duration::seconds(2), (None, None));
        map.insert(base_timestamp + Duration::seconds(3), (None, None));

        // Add more defined values to provide start and end points for interpolation
        map.insert(base_timestamp + Duration::seconds(4), (Some(130.0), None));
        map.insert(base_timestamp + Duration::seconds(5), (Some(150.0), None));
    }
    ...
    match calculate_volatility(&results_map, no_of_periods) {
        Some(volatility) => {
            ...
            for (timestamp, entry) in map.iter() {
                let vol = entry.1; // Assume the volatility value is in the fifth element
